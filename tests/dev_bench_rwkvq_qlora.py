...
print(f"loaded bf16 model in {time.time()-t0:.1f}s")
...
print(f"add_lora_rwkvq in {time.time()-t0:.1f}s, info={info}")
print(f"peak after add_lora_rwkvq: {mx.get_peak_memory()/1e9:.2f} GB")

# проверка: градиент реально течёт ТОЛЬКО в trainable (lora_a/lora_b)
train_params = dict(tree_flatten(model.trainable_parameters()))
print(f"trainable arrays: {len(train_params)}, names sample: {list(train_params.keys())[:4]}")
assert all(("lora_a" in k or "lora_b" in k) for k in train_params), "нашлись НЕ-lora trainable параметры!"
print("OK: trainable set == только lora_a/lora_b")
...
lcfg = LoRAConfig(lr=1e-4, max_steps=n_steps, grad_accum=1, grad_checkpoint=True,
                   ...
                   adapter_path="/tmp/bench_rwkvq_lora.safetensors")
