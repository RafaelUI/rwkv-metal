...
2. ~~**Реранкер**~~ — **сделано**, см. `docs/reranker.md`. Состояние прокинуто
   ...
   Попутно нашлась и **исправлена** утечка будущего в from-scratch
   архитектуре `RWKV7`: межблочный перенос token-shift (`x_prev = x[:, -1:]`
   после каждого блока) отдавал блоку i+1 ПОСЛЕДНИЙ токен чанка как
   «предыдущий». Возмущение сдвигается на позицию за слой, то есть задевало
   первые ~n_layer позиций окна (замер на 6 слоях: 0.209 на позиции 0, дальше
   меньше 0.007, после девятой — ничего; в x070 ровно 0.0). A/B на 400 шагов
   с нуля: val loss 5.73 legacy против 5.77 fixed — в пределах шума одного
   прогона. Чинилось не ради лосса, а потому что train и serve расходились и
   продолжение с состояния было невозможно определить. Теперь обе
   архитектуры каузальны и обе поддерживают state-API;
   `legacy_token_shift=True` воспроизводит старое поведение для уже
   обученных чекпоинтов.
