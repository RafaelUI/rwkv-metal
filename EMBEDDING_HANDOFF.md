...
1. **Настоящий прогон** — пока только 30-шаговые смоуки на 500 строках.
   Датасета 554K хватит надолго; куррикулум уже собран.
2. **Реранкер** — самая крупная оставшаяся часть. Требует прокинуть per-layer
   `(wkv_state, x_prev, v_first)` через `body()` / `RWKVBlock` / `Tmix` в обеих
   архитектурах, чтобы кэшировать state документа и досчитывать только запрос
   (O(L_query) вместо O(L_doc+L_query)). Фундамент есть:
   `make_wkv7_checkpoint_with_state` уже даёт дифференцируемый вход/выход `h_in`
   с полным VJP; формат state совпадает 1:1 с `state[1]` оригинала
   `[Layers, Batch, Heads, HeadSize, HeadSize]`. Наружу это не прокинуто —
   `make_wkv7_checkpoint` жёстко зануляет `h0`.
