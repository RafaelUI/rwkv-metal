...
# ─────────────────────────────────────────────────────────────────────────
# ЭКСПЕРИМЕНТ (03.08), по умолчанию ВЫКЛЮЧЕН: приводить выход WKV обратно к
# типу входа блока.
#
# Что течёт. Ядро WKV считает рекуррентность в fp32 — иначе нельзя — и
...
# поднимает ВЕСЬ вес до fp32. Течь начинается в tmix СЛОЯ 0: вход туда ещё
# bf16, выход уже fp32.
#
# Замерено в порту SwiftRWKV (та же архитектура, тот же MLX), 0.1B, release,
# A/B в одном процессе: декод 15.74 → 5.58 мс/ток (×2.82), перплексия на
# смешанном корпусе 16.8459 → 16.8610 (+0.090%). Для масштаба: пресет
# REDUCTION в rwkv-quant стоит +0.12% и считается там «около нуля».
#
# Почему ФЛАГОМ, а не просто исправлено: включение сдвигает числа во всём
# стеке, и rwkv-metal — источник эталонов паритета для SwiftRWKV. Менять
# источник надо согласованно и с ppl на руках, а не мимоходом.
#
#     import rwkv_metal.model.rwkv7_x070 as m
#     m.CAST_WKV_OUTPUT = True
#
# Флаг читается на КАЖДОМ проходе, поэтому обе ветки сравниваются в одном
# процессе — иначе замер ловил бы разницу прогонов, а не веток.
CAST_WKV_OUTPUT = False
...
class RWKV_Tmix_x070(nn.Module):
    ...
    def __call__(self, x, v_first, x_prev=None, h_in=None, mask=None,
        ...
        # ЕДИНСТВЕННАЯ точка, где fp32 из ядра входит в остаточный поток.
        # Приводится ВЫХОД, а не рекуррентность: h_out остаётся fp32 всегда,
        # иначе состояние потеряло бы точность и пошаговый путь разошёлся бы
        # со сплошным. См. CAST_WKV_OUTPUT наверху файла.
        if CAST_WKV_OUTPUT:
